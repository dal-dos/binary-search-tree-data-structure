...
public:
    ...
    void displayDeep(int k, ostream& out = cout) {
        if (isEmpty())
            out << "Empty tree" << endl;
        else
            displayDeep(root, k, out);
    }
...
private:
    ...
    void displayDeep(BinaryNode* t, int k, ostream& out) const
    {
        const int SHIFT = 4;
        if (t != nullptr)
        {
           if (k == 0) {
            ...
            displayDeep(t->left, k - 1, out);
            displayDeep(t->right, k - 1, out);
            }
        }
    }

    int countDeep(BinaryNode* t, int k) const
    {
        if (t != nullptr)
        {
            if (k == 0)
                ...
                return countDeep(t->left, k - 1) + countDeep(t->right, k - 1);
        }
        else {
            return 0;
        }
